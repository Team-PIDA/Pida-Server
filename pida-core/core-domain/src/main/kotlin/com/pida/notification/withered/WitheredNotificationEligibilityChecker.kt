...
/**
 * WITHERED 알림 적격성 검증 컴포넌트
 *
 * 특정 지역의 사용자 중 알림을 받을 수 있는 사용자를 필터링합니다.
 */
@Component
class WitheredNotificationEligibilityChecker(
    private val userReader: WitheredNotificationUserReader,
    private val regionResolver: WitheredRegionResolver,
     ...
     * - 올해 WITHERED_ALERT 알림을 받지 않은 사용자
     *
     * @param region 대상 지역
     * @return 적격 사용자 목록
     */
    fun findEligibleUsersForRegion(region: Region): List<EligibleUserWithRegion> {
        ...
        // 3. 올해 WITHERED_ALERT 알림 수신 여부 확인
        ...
        logger.info("${eligibleUsers.size} users have not received WITHERED_ALERT this year")
